...
export default function CreateListing() {
  ...
  const [files, setFiles] = useState([]);      // empty brackets because we need to have multiple files(images)
  ...
  useEffect(() => {
    const fetchListing = async () => {
         const listingId = params.listingId;   //inside app.jsx we called /update-listing/:listingId
  ...
  const handleImageSubmit = (e) => {
    if( files.length > 0 && files.length + formData.imageUrls.length < 7){
        ...
        const promises = [];       // we want to upload more than 1 images, therefore we have more than one async behaviour, so wait for all of them, one by one image uploaded to storage , so return more than one promise    }
        for(let i=0; i<files.length; i++){
            promises.push(storeImage(files[i]));   // all the downloadUrls are going to stored in this promise, now we want to save these downloadUrls inside a piece of state, and as we have more than one piece of state, like name description etc we want to create a form data
        }
        Promise.all(promises).then((urls) => {
            setFormData({...formData, imageUrls: formData.imageUrls.concat(urls), });
            setImageUploadError(false);
            // when loading complete set loading false
            setUploading(false);
        }).catch((error) => {
  ...
  const storeImage = async (file) => {     // async because we need to wait for the response from storage, pass them each file
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if(+formData.regularPrice < +formData.discountPrice) return setError('Discount Price must be lower than regular price');     // + for making sure the price that it is a number
