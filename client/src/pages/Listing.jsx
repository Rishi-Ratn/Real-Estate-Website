...
export default function Listing() {
    SwiperCore.use([Navigation]);  // use Navigation module for swiper
    ...
    const {currentUser} = useSelector((state) => state.user);           // destructured it to get the current user
    useEffect(() => {
    const fetchListing = async () => {
        try {
            ...
            // once we're successfull we'll set listing to data
            setListing(data);
            setLoading(false);
            setError(false);
        } catch (error) {
    ...
    fetchListing();
    }, [params.listingId]);       // run this useEffect each time params.listingId changes
    return (
      <main>
            ...
            <div className="flex gap-4">
                ...
                {listing.offer && (
                    <p className="bg-green-900 w-full max-w-[200px] text-white text-center p-1 rounded-md">
                        ${+listing.regularPrice - +listing.discountPrice} discount Applied
                    </p>
                )}
            </div>
